...
### Project Overview
Process batches from multiple branches into one durable store without duplicating records when a file is retried.

### Why This Project Matters
Real pipelines receive repeated and late files. Safe reruns, upserts, and an archive trail protect downstream data quality.

### Learning Objectives
Discover files, combine inputs, validate records, upsert idempotently, archive completed files, and test reruns.

### Architecture

```text
Incoming CSV batch → discover/extract → validate/transform → SQLite upsert → archive + logs
```

### Tools Used
**Python** coordinates stages, **pandas** handles batch transforms, **SQLite** demonstrates database upserts locally, and an **archive directory** records processed inputs.

### Prerequisites
Complete [Simple CSV ETL](../simple-csv-etl/) and read [ETL best practices](../../../notes/05-etl-and-data-pipelines/02-etl-best-practices.md).

### Setup and Project Tasks
Build sections 1–4, run all four scenarios in section 5, and explain why record counts prove idempotency.

### Expected Output
A queryable database containing the latest valid order state, archived inputs, rejected records where applicable, and stable counts after retries.

### Challenges / Extensions
Complete section 7, add unit tests, or schedule the pipeline after studying Airflow.
