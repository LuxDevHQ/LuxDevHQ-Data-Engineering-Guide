...
### Project Overview
Build a complete, runnable CSV pipeline that extracts messy sales data, validates and transforms it, and writes clean and rejected outputs.

### Why This Project Matters
Production source data is imperfect. Separating valid and invalid rows while logging every stage is a core Data Engineering habit.

### Learning Objectives
Practice modular ETL, pandas cleaning, validation, logging, rejected-record handling, and repeatable execution.

### Architecture

```text
Messy CSV → Python extract → pandas validate/transform → clean CSV + rejected CSV + logs
```

### Tools Used
**Python** runs the workflow, **pandas** transforms tabular data, **CSV files** make inputs and outputs inspectable, and **logging** records pipeline behavior.

### Prerequisites
Read [Python basics](../../../notes/04-python/01-python-basics.md) and [ETL/ELT workflows](../../../notes/05-etl-and-data-pipelines/01-etl-elt-workflows.md).

### Setup and Project Tasks
Complete sections 1–5 above in order, compare your run to the shown output, then attempt at least one extension from section 7.

### Expected Output
A successful log summary, a cleaned file with correct values and calculated columns, and a rejected-data file that explains what was excluded.

### Challenges / Extensions
Use the exercises already listed above; then add automated tests or load valid rows into PostgreSQL.
