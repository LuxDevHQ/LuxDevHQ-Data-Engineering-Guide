...
### Project Overview
Provision managed PostgreSQL, connect with DBeaver, create sample data, and document the path from stored input to query results.

### Why This Project Matters
Managed databases let a data engineer practice schemas and ingestion while the provider handles server maintenance.

### Learning Objectives
Provision a service safely, connect over TLS, run CRUD queries, and explain managed versus local PostgreSQL.

### Architecture

```text
Sample file → managed storage/integration → Aiven PostgreSQL → DBeaver
```

### Tools Used
**Aiven** hosts PostgreSQL; **PostgreSQL** stores relational data; **DBeaver** is the SQL client. Storage availability and product labels can change, so verify the original storage steps against current Aiven documentation before spending money.

### Prerequisites
Read [cloud fundamentals](../../../notes/02-cloud-and-linux/cloud-computing-fundamentals.md) and [PostgreSQL setup](../../../notes/03-sql-and-databases/postgresql-setup-and-sql-introduction.md).

### Setup and Project Tasks
Follow the original steps above, select a free/trial option only if currently available, never publish connection credentials, create the sample table, and capture safe evidence. Remove the service afterward if it can incur charges.

### Expected Output
A reachable database, the `products` table and queries, and the deliverables listed above with secrets removed.

### Challenges / Extensions
Load a CSV with `COPY`, add constraints, or automate safe connection configuration with environment variables.
